...
function handleLogout() {
    ...
    // Use postMessage to navigate in the Canvas environment
    ...
    function handleLogin() {
        ...
        if (users[username] && users[username].password === password) {
            ...
            setTimeout(() => {
                ...
                // Use postMessage to navigate in the Canvas environment
                window.parent.postMessage({ type: 'navigate', url: targetPage }, '*');
            }, 1000);
...
function setupSellerPage() {
    ...
    function handleListMaterial() {
        if (!currentUser) {
            messageBox.textContent = 'User not logged in. Please log in first.';
            messageBox.classList.remove('hidden');
            return;
        }
